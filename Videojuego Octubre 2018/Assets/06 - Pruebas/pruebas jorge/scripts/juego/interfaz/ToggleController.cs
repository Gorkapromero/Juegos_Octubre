...
public class ToggleController : MonoBehaviour 
{
	...
	void Start()
	{
		if(isOn)
		{
			...
			handleTransform.localPosition = new Vector3(onPosX, 5.2f, 0f);
		...
		else
		{
			...
			handleTransform.localPosition = new Vector3(offPosX, 5.2f, 0f);
	...
	Vector3 SmoothMove(GameObject toggleHandle, float startPosX, float endPosX)
	{
		...
		Vector3 position = new Vector3 (Mathf.Lerp(startPosX, endPosX, t += speed * Time.deltaTime), 5.2f, 0f);
		StopSwitching();
		return position;
	}
